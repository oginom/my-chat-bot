...
Tunables live in `src/config.ts` (`LIMITS`): history window up to 200 messages, capped at a cumulative 50000 chars walked back from the newest (whichever hits first — see `src/repository/message.ts`); stored-message truncation at 10000 chars; rate limits 5/min & 30/hr per channel. Change them there, not inline.
